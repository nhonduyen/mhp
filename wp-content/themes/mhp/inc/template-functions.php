 ...
 * @return array
 */
function mhp_body_classes( $classes ) {
	...
	if ( mhp_image_filters_enabled() ) {
		$classes[] = 'image-filters-enabled';
	}

	return $classes;
}
add_filter( 'body_class', 'mhp_body_classes' );

/**
 * Adds custom class to the array of posts classes.
 */
function mhp_post_classes( $classes, $class, $post_id ) {
	$classes[] = 'entry';

	return $classes;
}
add_filter( 'post_class', 'mhp_post_classes', 10, 3 );


/**
 * Add a pingback url auto-discovery header for single posts, pages, or attachments.
 */
function mhp_pingback_header() {
	if ( is_singular() && pings_open() ) {
		echo '<link rel="pingback" href="', esc_url( get_bloginfo( 'pingback_url' ) ), '">';
	}
}
add_action( 'wp_head', 'mhp_pingback_header' );

/**
 * Changes comment form default fields.
 */
function mhp_comment_form_defaults( $defaults ) {
...
add_filter( 'comment_form_defaults', 'mhp_comment_form_defaults' );

/**
 * Filters the default archive titles.
 */
function mhp_get_the_archive_title() {
	if ( is_category() ) {
		$title = __( 'Category Archives: ', 'mhp' ) . '<span class="page-description">' . single_term_title( '', false ) . '</span>';
	} elseif ( is_tag() ) {
		$title = __( 'Tag Archives: ', 'mhp' ) . '<span class="page-description">' . single_term_title( '', false ) . '</span>';
	} elseif ( is_author() ) {
		$title = __( 'Author Archives: ', 'mhp' ) . '<span class="page-description">' . get_the_author_meta( 'display_name' ) . '</span>';
	} elseif ( is_year() ) {
		$title = __( 'Yearly Archives: ', 'mhp' ) . '<span class="page-description">' . get_the_date( _x( 'Y', 'yearly archives date format', 'mhp' ) ) . '</span>';
	} elseif ( is_month() ) {
		$title = __( 'Monthly Archives: ', 'mhp' ) . '<span class="page-description">' . get_the_date( _x( 'F Y', 'monthly archives date format', 'mhp' ) ) . '</span>';
	} elseif ( is_day() ) {
		$title = __( 'Daily Archives: ', 'mhp' ) . '<span class="page-description">' . get_the_date() . '</span>';
	} elseif ( is_post_type_archive() ) {
		$title = __( 'Post Type Archives: ', 'mhp' ) . '<span class="page-description">' . post_type_archive_title( '', false ) . '</span>';
	} elseif ( is_tax() ) {
		$tax = get_taxonomy( get_queried_object()->taxonomy );
		/* translators: %s: Taxonomy singular name */
		$title = sprintf( esc_html__( '%s Archives:', 'mhp' ), $tax->labels->singular_name );
	} else {
		$title = __( 'Archives:', 'mhp' );
	}
	return $title;
}
add_filter( 'get_the_archive_title', 'mhp_get_the_archive_title' );
 ...
 */
function mhp_image_filters_enabled() {
 ...
 */
function mhp_post_thumbnail_sizes_attr( $attr ) {
...
add_filter( 'wp_get_attachment_image_attributes', 'mhp_post_thumbnail_sizes_attr', 10, 1 );

/**
 * Returns the size for avatars used in the theme.
 */
function mhp_get_avatar_size() {
	return 60;
}
 ...
 */
function mhp_is_comment_by_post_author( $comment = null ) {
 ...
 */
function mhp_get_discussion_data() {
 ...
 */
function mhp_add_ellipses_to_nav( $nav_menu, $args ) {
	...
	if ( 'menu-1' === $args->theme_location ) :
		...
		$nav_menu .= mhp_get_icon_svg( 'chevron_left' );
		$nav_menu .= esc_html__( 'Back', 'mhp' );
		$nav_menu .= '</button>';
		$nav_menu .= '</li>';
		$nav_menu .= '</ul>';
		$nav_menu .= '</li>';
...
add_filter( 'wp_nav_menu', 'mhp_add_ellipses_to_nav', 10, 2 );
 ...
 */
function mhp_nav_menu_link_attributes( $atts, $item, $args, $depth ) {
...
add_filter( 'nav_menu_link_attributes', 'mhp_nav_menu_link_attributes', 10, 4 );
 ...
 * Add a dropdown icon to top-level menu items
 */
function mhp_add_dropdown_icons( $output, $item, $depth, $args ) {
	...
	if ( in_array( 'mobile-parent-nav-menu-item', $item->classes, true ) && isset( $item->original_id ) ) {
		...
		$icon = mhp_get_icon_svg( 'keyboard_arrow_down', 24 );
...
add_filter( 'walker_nav_menu_start_el', 'mhp_add_dropdown_icons', 10, 4 );
 ...
 */
function mhp_add_mobile_parent_nav_menu_items( $sorted_menu_items, $args ) {
	static $pseudo_id = 0;
	if ( ! isset( $args->theme_location ) || 'menu-1' !== $args->theme_location ) {
...
add_filter( 'wp_nav_menu_objects', 'mhp_add_mobile_parent_nav_menu_items', 10, 2 );

/**
 * Convert HSL to HEX colors
 */
function mhp_hsl_hex( $h, $s, $l, $to_hex = true ) {
